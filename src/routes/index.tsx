...
function NeuroactividadPage() {
  ...
  return (
    <main className="overflow-hidden bg-paper pb-20 text-ink md:pb-0">
      ...
      <section className="relative h-[46vh] min-h-[280px] overflow-hidden sm:h-[56vh]">
        <img src="/family-band.jpg" alt="Una familia en casa, en un momento cotidiano de calma" className="absolute inset-0 h-full w-full object-cover" loading="lazy" />
        <div className="absolute inset-0 bg-gradient-to-t from-ink/80 via-ink/10 to-transparent" />
        <p className="section-shell absolute inset-x-0 bottom-8 font-display text-2xl italic text-paper sm:bottom-12 sm:text-3xl">Este libro nace para el día a día real de tu casa.</p>
      </section>
      ...
      <section className="relative overflow-hidden bg-ink py-24 text-center text-paper sm:py-32">
        <img src="/family-play.jpg" alt="Una madre jugando con sus hijos" className="absolute inset-0 h-full w-full object-cover opacity-25" loading="lazy" />
        <div className="absolute inset-0 bg-gradient-to-b from-ink via-ink/85 to-ink" />
        <Reveal className="section-shell relative"><Sparkles className="mx-auto text-coral"/><h2 className="mx-auto mt-5 max-w-4xl font-display text-5xl font-semibold leading-tight sm:text-7xl">No tienes que hacerlo todo bien</h2><p className="mx-auto mt-7 max-w-[55ch] text-xl leading-relaxed text-paper/75">Tu hijo no necesita perfección. Necesita un adulto suficientemente presente para observar, reparar y volver a intentarlo. La ciencia puede ayudarte a elegir el siguiente paso.</p><PurchaseLink className="mt-9">Empieza hoy <MoveRight size={19}/></PurchaseLink></Reveal></section>

      <footer className="bg-ink border-t border-paper/10 py-8 text-paper"><div className="section-shell flex flex-col items-center justify-between gap-2 font-ui text-xs text-paper/50 sm:flex-row"><span>NEUROACTIVIDAD · © 2026</span><div className="flex flex-wrap items-center justify-center gap-x-6 gap-y-1"><a href="#" className="hover:text-paper">Política de privacidad</a><a href="mailto:hola@neuroactividad.com" className="hover:text-paper">Contacto</a><span className="text-paper/30">Fotos: Freepik</span></div></div></footer>
